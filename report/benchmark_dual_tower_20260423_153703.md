...
| 表命中准确率 | **100.0%** |
| 平均检索延迟 | 1.3ms (SchemaGraph, 不含 Milvus) |
...
| Q04 | 查询包含超过20首歌的播放列表及所有歌曲的总时长 | 中 | 1,680 | 398 | 76% | 100% | 1 |
| Q05 | 找出购买过Jazz类型歌曲的所有客户姓名和邮箱 | 极高 | 1,680 | 962 | 43% | 100% | 2 |
| Q06 | 统计每种媒体格式下歌曲的平均时长和平均文件大小 | 低 | 1,680 | 316 | 81% | 100% | 0 |
| Q07 | 查询每位艺术家最畅销的专辑（按发票行数统计） | 高 | 1,680 | 535 | 68% | 100% | 1 |
...
- **Steiner Tree 路径**：Album → Track → Genre → Artist
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：Invoice → Customer → Employee
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：Track → Customer → InvoiceLine → Invoice → Genre
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：Album → Track → InvoiceLine → Artist
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：Album → Track → Artist → Playlist → PlaylistTrack
- **Before**：1,680 tokens
