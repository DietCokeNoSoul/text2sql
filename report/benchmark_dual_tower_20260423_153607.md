...
| 表命中准确率 | **100.0%** |
| 平均检索延迟 | 1.4ms (SchemaGraph, 不含 Milvus) |
...
| Q10 | 查询包含特定艺术家歌曲的所有播放列表名称及歌曲数量 | 极高 | 1,680 | 551 | 67% | 100% | 2 |
...
- **Steiner Tree 路径**：Customer → Employee → Invoice
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：Track → Genre → InvoiceLine → Customer → Invoice
- **Before**：1,680 tokens
...
- **Steiner Tree 路径**：PlaylistTrack → Track → Playlist → Artist → Album
- **Before**：1,680 tokens
