...
async def run_query_streaming_async(question: str, thread_id: str) -> None:
...
def run_query_streaming(question: str, thread_id: str) -> None:
    """同步入口：调用异步 token 级流式输出。

    对外提供与 run_query() 相同签名的同步接口，内部使用 asyncio 运行异步版本。
    """
    asyncio.run(run_query_streaming_async(question, thread_id))
