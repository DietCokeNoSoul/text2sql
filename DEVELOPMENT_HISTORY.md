...
28. [阶段二十四：CLI 接口](#28-阶段二十四cli-接口)
29. [阶段二十五：持久化对话记忆](#29-阶段二十五持久化对话记忆)
30. [阶段二十六：两级路由架构](#30-阶段二十六两级路由架构)
...
*最后更新：2026-05-09（新增第28-31章：CLI接口 / 持久化记忆 / 两级路由 / 语义查询缓存）*
...
Agent 此前只能通过 Python 代码调用，无法被其他 AI 工具（Claude Code、Copilot CLI 等）或普通用户直接使用。

### 解决方案

新建 `agent/cli.py`，注册 `text2sql` 全局命令，并通过 `pyproject.toml` 声明 entry point。

### 五大子命令

| 命令 | 说明 |
|------|------|
| `text2sql query "问题"` | 单次查询，节点级流式输出 |
| `text2sql stream "问题"` | 单次查询，token 级流式输出 |
| `text2sql chat` | 多轮交互式对话（支持 `--thread-id` 复用记忆）|
| `text2sql schema [TABLE]` | 显示数据库表结构 |
| `text2sql config` | 显示当前所有配置项 |

### 全局参数

```
--db URI          数据库连接串（覆盖 DB_URI 环境变量）
--provider        LLM 提供商
--model           模型名称
--api-key         API Key
--env FILE        加载指定 .env 文件
--thread-id ID    多轮会话记忆 ID
--log-level       日志级别
--json            机器可读 JSON 输出
--no-cache        禁用 LLM 缓存
```
...
# 使用
text2sql query "每个专辑有多少首歌？"
text2sql chat --thread-id my-session
...
| 文件 | 变更 |
|------|------|
| `agent/cli.py` | 新建：5 子命令 + 全局参数解析 + 惰性 graph 加载 |
| `pyproject.toml` | 新增 `[project.scripts]`、`[build-system]`、`[tool.setuptools.packages.find]` |
...
原有 `InMemorySaver` 在进程退出后丢失全部对话历史，无法跨会话保持上下文。

### 解决方案

将 `checkpointer` 从 `InMemorySaver` 改为 `SqliteSaver`，对话历史持久化写入本地 `checkpoints.db`。
...
原路由器只在三个 SQL Skill 之间做选择，无法处理与数据查询无关的普通对话（闲聊、打招呼、一般性提问），导致非数据库问题也会走完整的 DB 查询流程。
...
```
START
  └→ intent_router (L1)  — 判断是否需要查数据库
        ├─ general_chat  → general_chat_node（直接 LLM 回复）→ END
        └─ db_query      → skill_router (L2)  — 判断用哪个 Skill
                              ├─ simple_query  → END
                              ├─ complex_query → END
                              └─ data_analysis → END
```
...
- Prompt 极短（仅判断 general_chat / db_query）
- 传入最近 6 条消息作为上下文窗口，正确处理追问历史数据的情况
- AI 回复超 200 字符自动截断，节省 token
- 未知意图默认 `db_query`（宁可多查，不丢信息）

### L2 路由：Skill 选择

- 仅在 `db_query` 路径触发，和原路由器逻辑一致
- Skill 描述从 SKILL.md 动态加载（渐进式披露）
...
| "查询销售额前5的商品" | `db_query → simple_query` |
| 历史有 DB 结果，追问"那第一名呢" | `db_query`（读历史上下文）|
| 闲聊历史后问"它有多贵" | `general_chat`（历史无 DB 上下文）|

### 文件变更

| 文件 | 变更 |
|------|------|
| `agent/skill_graph_builder.py` | 新增 `intent_router`、`general_chat`、`skill_router` 三个节点；图结构改为两级条件跳转；新增 `_latest_human_message` 工具方法 |
| `agent/graph.py` | 更新 `_NODE_LABELS` 映射 |
...
LLM 精确响应缓存（SQLiteCache）只能命中完全相同的 prompt，无法处理语义相似的问题，例如：

- "数据库有多少张表？" ≠ "这个库里有几个表？"（字面不同，语义相同）

### 解决方案

新建 `agent/query_cache.py`，基于 sentence-transformers 余弦相似度实现语义级缓存：

```
新问题 → 向量化 → 搜索 SQLite 缓存库
  ├─ 相似度 ≥ 0.92 → 直接返回缓存答案（0 LLM 调用）
  └─ 未命中 → 走完整 graph → 结果写入缓存
```

### 三层缓存体系对比

| 层级 | 类型 | 命中条件 | 存储 | LLM 调用 |
|------|------|---------|------|---------|
| LLM 响应缓存 | 精确匹配 | prompt 完全一致 | `.langchain_cache.db` | 0 |
| **语义查询缓存** | **语义相似** | **余弦相似度 ≥ 0.92** | **`.query_cache.db`** | **0** |
| Schema 缓存 | 对象缓存 | 同一批表名 | 内存（TTL 5 分钟）| 减少 DB 访问 |

### 关键设计

- **复用模型**：和双塔检索共享同一 `sentence-transformers` 实例，无额外加载开销
- **持久化**：SQLite 文件，重启后缓存保留
- **TTL**：默认 7 天过期（`QUERY_CACHE_MAX_AGE_DAYS`）
- **容错**：读写失败只记录 warning，不中断主流程
- **集成点**：`run_query()` / `run_query_streaming()` 函数头部 lookup，尾部 store

### 配置项

| 环境变量 | 默认值 | 说明 |
|---------|--------|------|
| `QUERY_CACHE_THRESHOLD` | `0.92` | 余弦相似度阈值 |
| `QUERY_CACHE_MAX_AGE_DAYS` | `7` | 缓存有效期（天）|
| `LLM_CACHE_ENABLED` | `true` | 整体缓存开关（控制语义缓存启停）|

### 测试结果

```
"数据库里有多少张表？" 存入缓存
"这个数据库有几张表？" → 相似度 0.9636 → HIT ✅
"今天天气怎么样？"     → 相似度 < 0.92  → MISS ✅
```
...
| 文件 | 变更 |
|------|------|
| `agent/query_cache.py` | 新建：`SemanticQueryCache` 类 + `init_query_cache()` / `get_query_cache()` 单例 API |
| `agent/graph.py` | 启动时初始化语义缓存；`run_query` / `run_query_streaming_async` 加入 lookup + store |

---

## 32. 阶段二十八：Redis Schema 缓存

### 背景

Schema 缓存原本基于进程内存（`dict + TTL`），存在两个问题：
1. 进程重启后缓存清空，第一次查询仍需 100ms+ 的 DB IO
2. 多进程部署时各进程独立维护缓存，无法共享

### 解决方案

引入 `RedisSchemaCache`，让 Schema 缓存可选地持久化到 Redis，与原有内存缓存接口完全一致，支持热切换。

### 双后端架构

```
SCHEMA_CACHE_BACKEND=memory   →  SchemaCache（进程内 dict，原有行为）
SCHEMA_CACHE_BACKEND=redis    →  RedisSchemaCache（Redis，持久化 + 跨进程共享）
                                       ↓ Redis 不可达时自动降级
                                  SchemaCache（告警日志 + 继续运行）
```
...
| 键 | 内容 | TTL |
|----|------|-----|
| `text2sql:schema:table_names` | JSON 数组 `["tb_user", ...]` | 300s（可配）|
| `text2sql:schema:schema:tb_a,tb_b` | schema 文本（按表名排序拼接为 key）| 300s（可配）|
| `text2sql:schema:column_map` | JSON 对象 `{"表名": ["列名", ...]}` | 300s（可配）|

### 性能测试结果

| 场景 | 耗时 |
|------|------|
| 第 1 次（MISS，查 MySQL）| 151.4ms |
| 第 2 次（HIT，读 Redis）| 1.0ms |
| 提升倍数 | **~150x** |

### 容错机制

- 连接超时设为 2 秒（`socket_connect_timeout=2`），快速失败
- Redis 不可达 → 自动降级到内存缓存 + `WARNING` 日志，服务不中断
- 读写操作异常 → 只记录 `warning`，返回 `None`（触发正常 DB 查询路径）
...
SCHEMA_CACHE_REDIS_HOST=127.0.0.1
SCHEMA_CACHE_REDIS_PORT=6379
SCHEMA_CACHE_REDIS_DB=0
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `SCHEMA_CACHE_BACKEND` | `memory` | `memory` 或 `redis` |
| `SCHEMA_CACHE_TTL` | `300` | 缓存有效期（秒）|
| `SCHEMA_CACHE_REDIS_HOST` | `127.0.0.1` | Redis 主机地址 |
| `SCHEMA_CACHE_REDIS_PORT` | `6379` | Redis 端口 |
| `SCHEMA_CACHE_REDIS_DB` | `0` | Redis 数据库编号 |
| `SCHEMA_CACHE_REDIS_PASSWORD` | 空 | Redis 密码（无则留空）|
| `SCHEMA_CACHE_REDIS_PREFIX` | `text2sql:schema:` | Key 前缀，避免命名空间污染 |

### 文件变更

| 文件 | 变更 |
|------|------|
| `agent/config.py` | 新增 `SchemaCacheConfig` dataclass；`AgentConfig` 增加 `schema_cache` 字段 |
| `agent/database.py` | 新增 `RedisSchemaCache` 类；`SQLDatabaseManager` 增加 `schema_cache_config` 参数；新增 `_build_schema_cache()` 静态工厂方法 |
| `agent/skill_graph_builder.py` | `SQLDatabaseManager` 构造传入 `config.schema_cache` |
| `agent/cli.py` | `SQLDatabaseManager` 构造传入 `cfg.schema_cache` |
| `.env` / `.env.example` | 新增 Schema 缓存配置项 |
| `pyproject.toml` | 新增 `redis>=5.0` 依赖 |
...
分析缓存覆盖范围时发现，`get_column_map()` 未被任何缓存保护：

- **触发时机**：每次 SQL 纠错（`sql_correction.py`）都会调用它
- **实现方式**：用 SQLAlchemy `inspect` 逐表查询列信息，全库 11 张表约耗时 **103ms**
- **问题**：SQL 出错后纠错循环最多重试 3 次，每次都要重新 inspect，开销不必要

### 解决方案

将 `column_map` 纳入现有 Schema 缓存体系（`SchemaCache` / `RedisSchemaCache`），与表名、schema 共享同一 TTL 和 Redis 实例，无需引入新依赖。

### 缓存路径

```
get_column_map()
  ├─ schema_cache.get_column_map()  → Redis HIT → 直接返回 (0.5ms)
  └─ MISS → SQLAlchemy inspect 全库 (103ms) → schema_cache.set_column_map() → 写 Redis
```
...
| Key | 类型 | 内容 | TTL |
|-----|------|------|-----|
| `text2sql:schema:table_names` | JSON Array | 所有表名列表 | 300s |
| `text2sql:schema:column_map` | JSON Object | `{表名: [列名, ...]}` 全库列信息 | 300s |
| `text2sql:schema:schema:<tables>` | 纯文本 | 指定表组合的 DDL + 示例行 | 300s |

> `schema` key 按参与查询的表名排序后拼接，同一组表无论传入顺序如何都命中同一 key。

### 缓存 vs 非缓存边界

| 操作 | 是否缓存 | 原因 |
|------|---------|------|
| `get_table_names()` | ✅ Redis | 表结构极少变化 |
| `get_table_schema()` | ✅ Redis | 表结构极少变化 |
| `get_column_map()` | ✅ Redis（本次新增）| 表结构极少变化 |
| `execute_query(sql)` | ❌ 不缓存 | 查询结果实时变化，缓存会导致数据错误 |

### 性能测试结果

| 调用 | 耗时 | 说明 |
|------|------|------|
| 第 1 次（MISS，inspect 全库）| 103.1ms | 11 张表逐一 inspect |
| 第 2 次（Redis HIT）| 0.5ms | 快 **~200×** |
| 新实例冷启动（跨进程 HIT）| 0.5ms | 验证跨进程共享生效 |

### 文件变更

| 文件 | 变更 |
|------|------|
| `agent/database.py` | `SchemaCache`：新增 `_column_map_cache` 字段 + `get_column_map()` / `set_column_map()` 方法；`clear()` 同时清除列映射缓存 |
| `agent/database.py` | `RedisSchemaCache`：新增 `get_column_map()` / `set_column_map()`，key 为 `{prefix}column_map`，值为 JSON |
